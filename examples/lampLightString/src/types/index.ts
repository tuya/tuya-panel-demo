...
export interface RgbSceneValue {
  colors: IControlData[];
  id: number;
  interval: number;
  mode: number;
...
export interface RgbMusicColor {
  hue: number;
  saturation: number;
}
...
export interface BtnItemType {
  smearDisable: boolean | undefined;
  value?: string;
  icon?: string;
  image?: string;
  activedImage?: string;
...
export interface CloudTimerProps {
  id: string;
  index: number;
  title: string;
  isAdd: boolean;
