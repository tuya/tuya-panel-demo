...
class LineChart extends React.Component {
    constructor(props) {
      ...
      L ${pointsInChart[0].x - circleAddWidth - 2}, ${pointsInChart[0].y}`;
        ...
        this.getDashLineY = (param, yMax, yMin) => {
            ...
            switch (type) {
                ...
                case 'max':
                    ...
                    break;
                case 'custom':
                    const tempY = custom !== undefined ? (1 - (custom - yMin) / (yMax - yMin)) * gap + this.chartHeight * SvgMax : y;
        ...
        this.getDashLine = (yMax, yMin, FullChartWidth) => {
            ...
            if (!dashLine || !dashLine.dashLineConfig)
                return null;
            if (!dashLine.show)
                return null;
            let yShowText = [];
            const dashPath = dashLine.dashLineConfig.reduce((prev, current, index) => {
                ...
                yShowText.push(<react_native_1.Text style={showTextStyle} key={`yshowText_${index}`}>{`${showText}${dashLine.unit || ''}`}</react_native_1.Text>);
                return `${prev} M 0 ${chartYAxis} L${FullChartWidth} ${chartYAxis}`;
            }, '');
            return {
                dashline: <react_native_svg_1.Path d={dashPath} stroke={dashLine.color} strokeWidth="1" strokeDasharray="2,4" id='dashline' />,
                showText: yShowText,
            };
        };
        ...
        this.panResponder = react_native_1.PanResponder.create({
            ...
            onPanResponderRelease: () => { return false; },
            onPanResponderTerminate: () => { return false; },
        });
    ...
    render() {
        const { line, dashLine, hasShadow, hasCircle, tooltip, title, style, xData, yMax, yMin, activeCircle } = this.props;
        ...
        const y1 = this.chartHeight * SvgMax, y2 = this.chartHeight * SvgMin;
        const gap = y2 - y1;
        ...
        // title
        return (<react_native_1.View style={[styles.container, style]} {...this.panResponder.panHandlers}>
            <react_native_1.View>
                <react_native_1.View>
                    <react_native_svg_1.default width={FullChartWidth} height={this.chartHeight}>
                        <react_native_svg_1.Defs>
                            <react_native_svg_1.Path id="line" d={curvePath} stroke={lineColor} strokeWidth="2" fill={hasShadow ? "url(#shadow)" : "none"} />
                            {customDashLine ? customDashLine.dashline : null}
                            {tooltip && tooltip.show ? <react_native_svg_1.Path id='flag' d={this.getFlagPath(pointsInChart)} stroke='#EEEEEE' strokeWidth="1" /> : null}
                            {hasShadow ? <react_native_svg_1.LinearGradient x1="50%" y1="100%" x2="50%" y2="0%" id="shadow">
                                <react_native_svg_1.Stop stopColor={lineColor} stopOpacity="0" offset="0%"></react_native_svg_1.Stop>
                                <react_native_svg_1.Stop stopColor={lineColor} stopOpacity="0.100713315" offset="100%"></react_native_svg_1.Stop>
                            </react_native_svg_1.LinearGradient> : null}
                        </react_native_svg_1.Defs>
                        <react_native_svg_1.G>
                            <react_native_svg_1.Use href="#line" />
                            {hasCircle ? pointsInChart.map((it, index) => {
                                if (!activeCircle || (activeCircle && index === this.state.index)) {
                                    return (<react_native_svg_1.Circle key={index} cx={it.x} cy={it.y} r={circleRadius} strokeWidth={width} fill="white" stroke={lineColor} />);
                                }
                                return null;
                            }) : null}
                            {tooltip && tooltip.show ? <react_native_svg_1.Use href="#flag" /> : null}
                            {!isLine && (dashLine && dashLine.show) && (<react_native_svg_1.Use href="#dashline" />)}
                        </react_native_svg_1.G>
                    </react_native_svg_1.default>
                    {customDashLine ? customDashLine.showText : null}
                </react_native_1.View>
                {tooltip && tooltip.show ? (<tooltip_1.default title={title && typeof title.title === 'string' ? title.title : ''} {...tooltip} background={lineColor} chartWidth={FullChartWidth} chartHeight={this.chartHeight} right={right} yPoint={dataSource[this.state.index]} xPoint={xData && xData.length > 0 ? xData[this.state.index] : undefined} distance={Distance} />) : null}
