...
export default class DpsLists extends Component<DpsListsProps> {
  static propTypes = {
  ...
  constructor(props: DpsListsProps) {
    super(props);
    const dps = props.dps || [];
    const { dpState } = store.getState();
    this.state = {
      values: arrayToObject(
        dps.map(({ code }: { code: string }) => ({
          [code]: dpState[code],
        }))
      ),
    };
  ...
  getDpFunData() {
    ...
    return [
      {
        title: dpTitle,
        data: Object.keys(this.state.values).map((code: string) => {
  ...
  _handleItemValueChange(value: boolean, schema: any) {
    const mode = get(schema, 'mode');
  ...
  _handleItemPress(value: any, schema: any) {
