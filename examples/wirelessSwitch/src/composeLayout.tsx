    ...
    constructor(props: Props) {
      ...
      if (props && props.devInfo && props.devInfo.devId) {
        ...
        TYDevice.getDeviceInfo()
          .then(data => {
            const { schema } = data;
            const codes: any = _.filter(
              schema,
              d => /^switch/.test(d.code) || /^knob_switch_mode/.test(d.code)
            );
            codes.length > 0 && dispatch(actions.common.updateSelectCode(codes[0].code));
            codes.length > 0 && dispatch(actions.common.updateSelectValue(codes[0].range[0]));
            dispatch(actions.common.initSwitchCodes(codes));
