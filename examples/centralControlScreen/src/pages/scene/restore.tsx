...
const SceneRestore: FC = () => {
  const { hiddenSceneList } = useSelector(state => state);
  ...
  const save = () => {
    ...
    restoreResource(EResourceType.scene, selectedSceneIds)
      .then(() => {
        getData();
        TYSdk.mobile.hideLoading();
  ...
  const renderTopBar = () => {
    return (
      <TopBar
        title={Strings.getLang('sceneRestore')}
