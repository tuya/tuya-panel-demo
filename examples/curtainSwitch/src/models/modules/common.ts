...
export const switchCodesGet = createAction('_SWITCHCODESGET_');
...
const switchCodes = handleActions(
  {
    [switchCodesGet.toString()]: (state = {}, action = {}) => {
      const schema = TYDevice.getDpSchema();
      const keys = Object.keys(schema);
      const switchKeys = keys.filter(
        data => /^control/.test(data) && data !== 'control_back' && data !== 'control_back_2'
      );
      const switchs = _pick(schema, switchKeys);
      return {
        ...switchs,
      };
    },
