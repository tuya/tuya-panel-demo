...
import { getDpName } from './models/modules/cloudState';
      ...
      if (props && props.devInfo && props.devInfo.devId) {
        ...
        TYDevice.getDeviceInfo()
          .then(data => {
            ...
            const switcheCodes = _.filter(
              schema,
              d =>
                /^control?/.test(d.code) && d.code !== 'control_back' && d.code !== 'control_back_2'
            );
            dispatch(actions.common.devInfoChange(data));
            dispatch(actions.common.switchCodesGet());
            dispatch(actions.cloud.getDpName({ codes: switcheCodes.map(d => d.code) }) as any);
